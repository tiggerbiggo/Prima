...
public class FileManager {
  ...
  public static SafeImage[] getImgsFromVideo(File f){
    try {
      FrameGrab grab = FrameGrab.createFrameGrab(NIOUtils.readableChannel(f));
      List<SafeImage> imgs = new ArrayList<>();
      ...
      try {
        while (null != (picture = grab.getNativeFrame())) {
          imgs.add(new SafeImage(AWTUtil.toBufferedImage(picture)));
        }
      }
      catch(NullPointerException ex){

      }
      return imgs.toArray(new SafeImage[]{});
    }
