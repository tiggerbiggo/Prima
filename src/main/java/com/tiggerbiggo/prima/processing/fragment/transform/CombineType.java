...
public enum CombineType implements Serializable {
  ADD {
    @Override
    public Vector2 combine(Fragment<Vector2> A, Fragment<Vector2> B, int x, int y, int w, int h,
        int num) {
      return Vector2.add(
          A.get(x, y, w, h, num),
          B.get(x, y, w, h, num)
  ...
  public abstract Vector2 combine(Fragment<Vector2> A, Fragment<Vector2> B, int x, int y, int w,
      int h, int num);
}
