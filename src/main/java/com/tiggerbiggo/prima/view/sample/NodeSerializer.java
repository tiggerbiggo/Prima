...
public class NodeSerializer {
  public static final String PARSER_VERSION_STRING = "|Prim-p7|";
  ...
  * */
  ...
  public static String WARN_MESSAGE = "This file was created using a different version of Prima to the one you are currently using. Prima can attempt to convert these changes, but errors or corruption may occur. Do you want to continue?";

  public static NodePane parseFromFile(File file, ChangeListener listener) throws NodeParseException{
    try {
      List<String> lines = Files.readAllLines(file.toPath(), Charset.defaultCharset());
      if(lines.get(0).equals(PARSER_VERSION_STRING)){
        //all is well, remove the first element and start parsing
        lines.remove(0);
      }
      else{
        //Differing version, may be incompatible.

        Alert a = new Alert(AlertType.WARNING, WARN_MESSAGE, ButtonType.YES, ButtonType.NO);
        try{
      ...
      if(lines.get(0).startsWith("|")) lines.remove(0);
      //remove any version strings.
      return parseNodes(join(lines, "\n"), listener);
    } catch (IOException e) {
      return null;
    }
  }
  ...
  public static String saveToFile(NodePane pane, File toWrite) throws IOException {
    ...
    try(FileWriter fw = new FileWriter(toWrite)) {
      fw.append(PARSER_VERSION_STRING + "\n");
      fw.append(ser);
    } catch (IOException e) {
      throw e;
    }
